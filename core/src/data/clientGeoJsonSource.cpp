...
using vt_feature = mapbox::geojsonvt::detail::vt_feature;
using GeoJSONVT = mapbox::geojsonvt::GeoJSONVT;
...
const geojsonvt::Options tileOptions = { 18, 5, 100000, false, tolerance, tileExtent, tileBuffer };

struct ClientGeoJsonData {
    std::unique_ptr<GeoJSONVT> tiles;
...
void ClientGeoJsonSource::addPoint(const Properties& _tags, LngLat _point) {
    ...
    store->features.push_back(geometry::feature<double>{std::move(geom), {}, id });
...
void ClientGeoJsonSource::addData(const std::string& _data) {
    ...
    for (auto& feature : features) {
        ...
        feature.id = store->properties.size();
        store->properties.emplace_back();
        Properties& props = store->properties.back();
